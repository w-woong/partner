...
import (
	...
	"github.com/w-woong/partner/adapter"
	"github.com/w-woong/partner/cmd/http/route"
	"github.com/w-woong/partner/port"
	"github.com/w-woong/partner/usecase"
	"gorm.io/gorm"
...
func init() {
	flag.StringVar(&addr, "addr", ":49004", "listen address")
...
func main() {
	...
	// service
	var addrSvc port.AddressSvc
	switch conf.Partner.Address.Type {
	case "kakao":
		addrSvc = adapter.NewAddressSvcKakao(conf.Partner.Address.HtmlFile)
	default:
		logger.Error(conf.Partner.Address.Type + " is not allowed")
		os.Exit(1)
	}
	addrUsc := usecase.NewAddressFinder(addrSvc)
	...
	route.AddressRoute(router, conf.Server.Http, idTokenCookie, idTokenParser, userSvc, addrUsc)
